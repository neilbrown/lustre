				     ...
				     struct ksock_conn *conn)
{
	...
	if (route->ksnr_myiface != conn_iface) {
		if (route->ksnr_myiface < 0) {
			...
			CDEBUG(D_NET, "Rebinding %s %pIS from interface %d to %d\n",
			       libcfs_id2str(peer_ni->ksnp_id),
			       &route->ksnr_addr,
			       route->ksnr_myiface,
			       conn_iface);
...
ksocknal_local_ipvec(struct lnet_ni *ni, __u32 *ipaddrs)
{
	...
	for (i = 0, j = 0; i < nip; i++)
		if (net->ksnn_interfaces[i].ksni_addr.ss_family == AF_INET) {
			...
			ipaddrs[i] = ntohl(sa->sin_addr.s_addr);
			LASSERT(ipaddrs[i] != 0);
		}
...
ksocknal_match_peerip(struct ksock_interface *iface, __u32 *ips, int nips)
{
	...
	LASSERT(iface->ksni_addr.ss_family == AF_INET);
...
ksocknal_finalize_zcreq(struct ksock_conn *conn)
{
	...
	while (!list_empty(&zlist)) {
		tx = list_first_entry(&zlist, struct ksock_tx, tx_zc_list);
...
ksocknal_free_buffers (void)
{
	...
	if (!list_empty(&ksocknal_data.ksnd_idle_noop_txs)) {
		...
		while (!list_empty(&zlist)) {
			tx = list_first_entry(&zlist, struct ksock_tx, tx_list);
			list_del(&tx->tx_list);
