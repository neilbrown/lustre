                                                ...
                                                struct ldlm_extent *new_ex)
{
	...
	for (idx = 0; idx < LCK_MODE_NUM; idx++) {
		...
		if (conflicting > 4 ||
		    !INTERVAL_TREE_EMPTY(&tree->lit_root))
			/* tree is no empty, so don't expand backwards */
			new_ex->start = req_start;

		lck = extent_iter_first(&tree->lit_root, req_end + 1, U64_MAX);
		/* lck is the lock with the lowest endpoint which
		 * covers anything after 'req'
		 */
		if (lck)
			new_ex->end = min(new_ex->end,
					  lck->l_policy_data.l_extent.start - 1);

		if (extent_iter_first(&tree->lit_root, req_start, req_end))
			CDEBUG(D_INFO,
