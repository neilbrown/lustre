...
nrs_tbf_expression_parse(char *str, struct list_head *cond_list)
{
	...
	str = strim(str);
	len = strlen(str);
	if (!*field || len < 2 || str[0] != '{' || str[len-1] != '}')
		GOTO(out, rc = -EINVAL);
...
nrs_tbf_conjunction_parse(char *str, struct list_head *cond_list)
{
	...
	while (str && *str) {
		char *expr = strim(strsep(&str, NRS_TBF_CONJUNCTION_DELIM));
		if (!*expr) {
			rc = -EINVAL;
			break;
		}
...
nrs_tbf_conds_parse(char *orig, struct list_head *cond_list)
{
	...
	for (str = strim(orig); *str ; str = strim(str)) {
		char *term = strim(strsep(&str, NRS_TBF_DISJUNCTION_DELIM));
